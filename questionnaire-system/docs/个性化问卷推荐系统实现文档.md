# 个性化问卷推荐系统实现文档

## 📋 概述

本文档详细描述智能问卷分析系统中基于用户标签的个性化问卷推荐功能的实现原理、算法设计和技术细节。

### 核心功能

- 基于用户兴趣标签的智能推荐
- 多维度综合评分算法
- 实时动态推荐排序
- 支持无标签用户的随机推荐

---

## 🎯 推荐算法设计

### 算法原理

个性化推荐系统采用**优先级排序 + 混合推荐算法**，排序策略如下：

**优先级排序（主要）**：

1. **完全匹配优先** - 用户所有标签都在问卷标签中的问卷排最前
2. **部分匹配排序** - 按匹配标签数量降序排列
3. **综合评分排序** - 匹配数量相同时按综合得分排序

**综合评分计算（辅助）**：

1. **内容相似度** (Content-based Filtering) - 50%权重
2. **质量评分** (Quality Score) - 30%权重
3. **热度指标** (Popularity Score) - 20%权重

### 推荐得分计算公式

```javascript
// 综合评分（用于同优先级问卷的排序）
recommendScore = tagScore × 0.5 + ratingScore × 0.3 + popularityScore × 0.2
```

其中：

- `tagScore`: 用户标签与问卷标签的匹配度（匹配数量/用户标签总数）
- `ratingScore`: 问卷评分归一化值（rating/5）
- `popularityScore`: 参与人数归一化值（min(参与数/1000, 1)）
- `matchCount`: 匹配的标签数量（用于优先级排序）
- `isFullMatch`: 是否完全匹配（用于优先级排序）

---

## 💻 核心代码实现

### 1. 推荐逻辑主函数

**位置**: `client/src/hooks/useListFilter.js`（主要实现）  
**辅助位置**: `client/src/composables/useHomeLogic.js`（已废弃的旧实现）

```javascript
else if (sortBy.value === 'recommended') {
  // 智能推荐：根据用户标签进行相关性推荐
  const userTags = userStore.profile?.tags || [];
  console.log("[推荐系统] 用户标签:", userTags);

  if (userTags && userTags.length > 0) {
    // 有标签时：计算相关性得分
    result.forEach((survey) => {
      const surveyTags = survey.tags || [];

      // 计算标签匹配度
      const matchCount = surveyTags.filter((tag) =>
        userTags.includes(tag)
      ).length;
      const tagScore = matchCount / Math.max(userTags.length, 1);

      // 判断是否完全匹配(用户所有标签都在问卷标签中)
      const isFullMatch = userTags.every(tag => surveyTags.includes(tag));

      // 综合评分：标签匹配(50%) + 评分(30%) + 参与人数(20%)
      survey.recommendScore =
        tagScore * 0.5 +
        ((survey.rating || 0) / 5) * 0.3 +
        Math.min(
          (survey.participantCount || survey.participants || 0) / 1000,
          1
        ) * 0.2;

      // 存储匹配信息用于排序
      survey.matchCount = matchCount;
      survey.isFullMatch = isFullMatch;
    });

    // 按优先级排序：完全匹配 > 部分匹配(按匹配数量) > 无匹配(按评分)
    result.sort((a, b) => {
      // 优先级1: 完全匹配的排最前
      if (a.isFullMatch && !b.isFullMatch) return -1;
      if (!a.isFullMatch && b.isFullMatch) return 1;

      // 优先级2: 匹配标签数量多的在前
      if (a.matchCount !== b.matchCount) {
        return b.matchCount - a.matchCount;
      }

      // 优先级3: 匹配数量相同时,按综合得分排序
      return (b.recommendScore || 0) - (a.recommendScore || 0);
    });

    console.log(
      "[推荐系统] 基于标签匹配推荐，前5个问卷:",
      result.slice(0, 5).map((s) => ({
        title: s.title,
        tags: s.tags,
        matchCount: s.matchCount,
        isFullMatch: s.isFullMatch,
        score: s.recommendScore,
      }))
    );
  } else {
    // 无标签时：随机打乱顺序展示
    result = result.sort(() => Math.random() - 0.5);
    console.log("[推荐系统] 用户无标签，随机推荐");
  }
}
```

---

## 📊 算法详细说明

### 2.1 标签匹配度计算 (Tag Score)

**目的**: 衡量用户兴趣与问卷内容的相关性

**计算方法**:

```javascript
const matchCount = surveyTags.filter((tag) => userTags.includes(tag)).length;
const tagScore = matchCount / Math.max(userTags.length, 1);
```

**示例**:

```javascript
// 用户标签
userTags = ["技术", "编程", "前端"]

// 问卷A标签
surveyA.tags = ["技术", "编程"]
matchCount = 2
tagScore = 2 / 3 = 0.667

// 问卷B标签
surveyB.tags = ["技术", "后端", "Java"]
matchCount = 1
tagScore = 1 / 3 = 0.333
```

**权重**: 50% (最高权重，体现内容相关性的重要性)

### 2.2 质量评分 (Rating Score)

**目的**: 确保推荐高质量的问卷

**计算方法**:

```javascript
const ratingScore = (survey.rating || 0) / 5;
```

**归一化**: 将评分从 0-5 范围转换为 0-1 范围

**示例**:

```javascript
// 问卷评分为 4.5
ratingScore = 4.5 / 5 = 0.9

// 问卷评分为 3.0
ratingScore = 3.0 / 5 = 0.6
```

**权重**: 30%

### 2.3 热度指标 (Popularity Score)

**目的**: 平衡新问卷与热门问卷，避免冷启动问题

**计算方法**:

```javascript
const popularityScore = Math.min(
  (survey.participantCount || survey.participants || 0) / 1000,
  1
);
```

**归一化**: 以 1000 人为满分基准，超过 1000 人参与的问卷得分为 1

**示例**:

```javascript
// 500人参与
popularityScore = Math.min(500 / 1000, 1) = 0.5

// 1500人参与
popularityScore = Math.min(1500 / 1000, 1) = 1.0
```

**权重**: 20%

---

## 🔄 完整推荐流程

### 流程图

```
开始
  ↓
检查用户是否有标签
  ↓
有标签? ──── 否 ───→ 随机打乱问卷列表 → 返回结果
  ↓
  是
  ↓
遍历所有问卷
  ↓
计算每个问卷的三个维度得分:
  - 标签匹配度 (50%)
  - 质量评分 (30%)
  - 热度指标 (20%)
  ↓
计算综合推荐得分
  ↓
按推荐得分降序排序
  ↓
记录日志并返回排序后的问卷列表
  ↓
结束
```

### 具体步骤

1. **获取用户标签**

   ```javascript
   const userTags = userStore.profile?.tags || [];
   ```

2. **判断用户是否有标签**

   - 有标签 → 执行智能推荐算法
   - 无标签 → 随机推荐

3. **遍历问卷列表计算得分**

   ```javascript
   filtered.forEach((survey) => {
     // 计算各维度得分
     // 加权求和得到最终得分
   });
   ```

4. **优先级排序**

   ```javascript
   result.sort((a, b) => {
     // 优先级1: 完全匹配的排最前
     if (a.isFullMatch && !b.isFullMatch) return -1;
     if (!a.isFullMatch && b.isFullMatch) return 1;

     // 优先级2: 匹配标签数量多的在前
     if (a.matchCount !== b.matchCount) {
       return b.matchCount - a.matchCount;
     }

     // 优先级3: 匹配数量相同时,按综合得分排序
     return (b.recommendScore || 0) - (a.recommendScore || 0);
   });
   ```

5. **返回结果**
   - 前端展示前 20 个推荐问卷
   - 控制台输出前 3 个问卷的推荐详情

---

## 🎨 前端集成

### 在首页中的使用

**文件**: `client/src/views/frontend/home/HomePage.vue`

```vue
<script setup>
import { useHomeLogic } from "@/composables/useHomeLogic";
import { useListFilter } from "@/hooks/useListFilter";

// 获取问卷数据和推荐逻辑
const { surveys, userStore } = useHomeLogic();

// 使用列表过滤器
const { sortBy, filteredList, handleSort } = useListFilter({
  sourceList: surveys,
  searchFields: ["searchText"],
});

// 设置默认为推荐排序
sortBy.value = "recommended";
handleSort();

// 限制首页显示前20条
const topSurveys = computed(() => (filteredList.value || []).slice(0, 20));
</script>
```

### 排序选项

系统支持三种排序方式：

```javascript
const sortOptions = [
  { label: "推荐", value: "recommended" }, // 智能推荐
  { label: "最新", value: "latest" }, // 按创建时间
  { label: "最热", value: "hot" }, // 按参与人数
];
```

---

## 📈 推荐效果示例

### 示例场景

**用户信息**:

```json
{
  "username": "tech_enthusiast",
  "tags": ["技术", "编程", "AI"]
}
```

**问卷列表**:

| 问卷 ID | 标题         | 标签             | 评分 | 参与人数 |
| ------- | ------------ | ---------------- | ---- | -------- |
| S1      | AI 技术调研  | ["AI", "技术"]   | 4.5  | 800      |
| S2      | 前端开发调查 | ["编程", "前端"] | 4.0  | 600      |
| S3      | 市场调研问卷 | ["市场", "用户"] | 3.5  | 1200     |

**推荐得分计算**:

**问卷 S1 (AI 技术调研)**:

- 标签匹配: 2/3 = 0.667
- 质量评分: 4.5/5 = 0.9
- 热度指标: 800/1000 = 0.8
- **总得分**: 0.667×0.5 + 0.9×0.3 + 0.8×0.2 = **0.764**

**问卷 S2 (前端开发调查)**:

- 标签匹配: 1/3 = 0.333
- 质量评分: 4.0/5 = 0.8
- 热度指标: 600/1000 = 0.6
- **总得分**: 0.333×0.5 + 0.8×0.3 + 0.6×0.2 = **0.527**

**问卷 S3 (市场调研问卷)**:

- 标签匹配: 0/3 = 0
- 质量评分: 3.5/5 = 0.7
- 热度指标: 1000/1000 = 1.0
- **总得分**: 0×0.5 + 0.7×0.3 + 1.0×0.2 = **0.410**

**优先级排序结果**: S1 > S2 > S3

**排序说明**:

- S1: 匹配 2 个标签（AI、技术）
- S2: 匹配 1 个标签（编程）
- S3: 匹配 0 个标签

根据优先级规则，匹配标签越多，排序越靠前。

---

## 🔍 调试与日志

### 控制台日志

推荐系统会在浏览器控制台输出详细的调试信息：

```javascript
// 1. 用户标签
console.log("[推荐系统] 用户标签:", userTags);
// 输出: [推荐系统] 用户标签: ["技术", "编程", "AI"]

// 2. 推荐结果（前3个）
console.log(
  "[推荐系统] 基于标签匹配推荐，前3个问卷:",
  filtered.slice(0, 3).map((s) => ({
    title: s.title,
    tags: s.tags,
    score: s.recommendScore,
  }))
);
// 输出:
// [推荐系统] 基于标签匹配推荐，前3个问卷: [
//   { title: "AI技术调研", tags: ["AI", "技术"], score: 0.764 },
//   { title: "前端开发调查", tags: ["编程", "前端"], score: 0.527 },
//   ...
// ]

// 3. 无标签情况
console.log("[推荐系统] 用户无标签，随机推荐");
```

---

## 🎯 优化建议

### 当前实现的优势

✅ **简单高效**: 计算复杂度 O(n)，适合中小规模数据集  
✅ **可解释性强**: 推荐结果有明确的得分依据  
✅ **实时计算**: 无需预处理，动态响应用户变化  
✅ **兼容性好**: 支持有标签和无标签用户

### 未来可优化方向

#### 1. 协同过滤 (Collaborative Filtering)

```javascript
// 基于用户行为的协同过滤
const userBehaviorScore = calculateCollaborativeScore(
  userId,
  surveyId,
  userBehaviorMatrix
);

// 混合推荐
survey.recommendScore =
  tagScore * 0.4 +
  ratingScore * 0.2 +
  popularityScore * 0.1 +
  userBehaviorScore * 0.3;
```

#### 2. 时间衰减因子

```javascript
// 考虑问卷发布时间，新问卷获得加成
const daysSincePublished =
  (Date.now() - new Date(survey.publishedAt)) / (1000 * 60 * 60 * 24);
const timeDecayFactor = Math.exp(-daysSincePublished / 30); // 30天半衰期

survey.recommendScore = baseScore * (1 + timeDecayFactor * 0.2);
```

#### 3. 用户历史行为

```javascript
// 避免推荐用户已完成的问卷
const completedSurveys = userStore.profile?.completedSurveys || [];
filtered = filtered.filter((s) => !completedSurveys.includes(s.id));

// 提升用户收藏类似问卷的推荐
const favoriteTags = getFavoritesTags(userStore.favorites);
const favoriteBoost = calculateFavoriteBoost(survey.tags, favoriteTags);
survey.recommendScore += favoriteBoost * 0.1;
```

#### 4. A/B 测试框架

```javascript
// 多种推荐策略对比
const strategies = {
  tagBased: { weights: [0.5, 0.3, 0.2] },
  popularityBased: { weights: [0.2, 0.3, 0.5] },
  balanced: { weights: [0.33, 0.33, 0.34] },
};

const userStrategy = getUserStrategy(userId); // A/B分组
applyRecommendStrategy(strategies[userStrategy]);
```

#### 5. 机器学习模型

```javascript
// 使用训练好的模型预测用户兴趣
const features = extractFeatures(user, survey);
const mlScore = await predictUserInterest(features);

survey.recommendScore = traditionScore * 0.6 + mlScore * 0.4;
```

---

## 📊 性能考虑

### 当前性能特点

- **时间复杂度**: O(n × m)
  - n: 问卷数量
  - m: 平均标签数量
- **空间复杂度**: O(n)
  - 需要为每个问卷存储 recommendScore

### 性能优化建议

#### 1. 缓存机制

```javascript
// 缓存用户推荐结果（5分钟）
const cacheKey = `recommend_${userId}_${Date.now() / 300000}`;
const cached = localStorage.getItem(cacheKey);
if (cached) return JSON.parse(cached);
```

#### 2. 分页加载

```javascript
// 只计算当前页需要的问卷
const pageSize = 20;
const topCandidates = filtered
  .slice(0, 100) // 只取前100个候选
  .map(calculateScore)
  .sort()
  .slice(0, pageSize);
```

#### 3. Web Worker

```javascript
// 将推荐计算放到 Web Worker
const worker = new Worker("/workers/recommend.js");
worker.postMessage({ surveys, userTags });
worker.onmessage = (e) => {
  const recommendations = e.data;
  updateUI(recommendations);
};
```

---

## 🧪 测试用例

### 单元测试示例

```javascript
describe("推荐系统测试", () => {
  test("有标签用户应该获得个性化推荐", () => {
    const user = { tags: ["技术", "编程"] };
    const surveys = [
      { id: 1, tags: ["技术"], rating: 4.0, participantCount: 500 },
      { id: 2, tags: ["市场"], rating: 4.5, participantCount: 800 },
    ];

    const result = recommend(user, surveys);

    expect(result[0].id).toBe(1); // 技术问卷排第一
    expect(result[0].recommendScore).toBeGreaterThan(result[1].recommendScore);
  });

  test("无标签用户应该获得随机推荐", () => {
    const user = { tags: [] };
    const surveys = [...Array(10)].map((_, i) => ({ id: i }));

    const result1 = recommend(user, surveys);
    const result2 = recommend(user, surveys);

    expect(result1).not.toEqual(result2); // 两次结果应该不同
  });

  test("推荐得分计算正确性", () => {
    const survey = {
      tags: ["技术", "编程"],
      rating: 4.0,
      participantCount: 600,
    };
    const userTags = ["技术", "编程", "AI"];

    const score = calculateRecommendScore(survey, userTags);

    // tagScore: 2/3 = 0.667
    // ratingScore: 4/5 = 0.8
    // popularityScore: 600/1000 = 0.6
    // expected: 0.667*0.5 + 0.8*0.3 + 0.6*0.2 = 0.6935
    expect(score).toBeCloseTo(0.6935, 2);
  });
});
```

---

## 📝 数据结构说明

### 用户标签数据结构

```javascript
// users 表中的 tags 字段 (JSON)
{
  "id": "user_123",
  "username": "techfan",
  "tags": [
    "技术",
    "编程",
    "AI",
    "前端",
    "数据分析"
  ]
}
```

### 问卷标签数据结构

```javascript
// surveys 表中的 tags 字段 (JSON)
{
  "id": "survey_456",
  "title": "AI技术应用调研",
  "tags": [
    "AI",
    "技术",
    "机器学习"
  ],
  "rating": 4.5,
  "participantCount": 850
}
```

### 推荐结果数据结构

```javascript
// 添加了 recommendScore 的问卷对象
{
  "id": "survey_456",
  "title": "AI技术应用调研",
  "tags": ["AI", "技术", "机器学习"],
  "rating": 4.5,
  "participantCount": 850,
  "recommendScore": 0.764,  // 推荐得分
  // ... 其他字段
}
```

---

## 🔧 配置参数

### 可调整参数

```javascript
// 在 useHomeLogic.js 中
const RECOMMEND_CONFIG = {
  // 权重配置
  TAG_WEIGHT: 0.5, // 标签匹配权重
  RATING_WEIGHT: 0.3, // 质量评分权重
  POPULARITY_WEIGHT: 0.2, // 热度权重

  // 归一化参数
  MAX_RATING: 5, // 最大评分
  POPULARITY_BASE: 1000, // 热度基准人数

  // 显示配置
  HOMEPAGE_LIMIT: 20, // 首页显示数量
  LOG_TOP_N: 3, // 日志显示前N个
};
```

---

## 📖 相关文件

### 核心文件

| 文件路径                                      | 说明                         |
| --------------------------------------------- | ---------------------------- |
| `client/src/hooks/useListFilter.js`           | **推荐算法核心实现**（主要） |
| `client/src/composables/useHomeLogic.js`      | 数据加载和业务逻辑           |
| `client/src/views/frontend/home/HomePage.vue` | 首页推荐展示                 |
| `client/src/store/user.js`                    | 用户数据和标签管理           |

### 数据模型

| 文件路径                  | 说明                       |
| ------------------------- | -------------------------- |
| `server/models/User.js`   | 用户模型（包含 tags 字段） |
| `server/models/Survey.js` | 问卷模型（包含 tags 字段） |

---

## 💡 最佳实践

### 1. 标签管理

```javascript
// 推荐使用统一的标签库
const TAG_LIBRARY = [
  "技术",
  "编程",
  "AI",
  "数据分析",
  "市场",
  "用户研究",
  "产品",
  "教育",
  "健康",
  "生活",
];

// 标签标准化
const normalizeTags = (tags) => {
  return tags
    .map((tag) => tag.trim())
    .filter((tag) => TAG_LIBRARY.includes(tag));
};
```

### 2. 用户标签更新

```javascript
// 根据用户行为自动更新标签
const updateUserTags = async (userId, surveyTags) => {
  const user = await getUser(userId);
  const currentTags = user.tags || [];

  // 合并并去重
  const newTags = [...new Set([...currentTags, ...surveyTags])];

  // 限制标签数量（最多10个）
  const limitedTags = newTags.slice(0, 10);

  await updateUser(userId, { tags: limitedTags });
};
```

### 3. 冷启动处理

```javascript
// 新用户首次访问时推荐热门问卷
if (!userTags || userTags.length === 0) {
  // 方案1: 随机推荐
  filtered = filtered.sort(() => Math.random() - 0.5);

  // 方案2: 推荐热门问卷
  // filtered.sort((a, b) =>
  //   (b.participantCount || 0) - (a.participantCount || 0)
  // );
}
```

---

## 📚 参考资料

### 推荐系统理论

- **协同过滤**: User-based & Item-based Collaborative Filtering
- **内容推荐**: Content-based Recommendation
- **混合推荐**: Hybrid Recommendation Systems
- **评价指标**: Precision, Recall, NDCG, MAP

### 相关技术

- Vue 3 Composition API
- Pinia 状态管理
- JavaScript 数组操作
- 浏览器控制台调试

---

## 📞 联系与反馈

如有问题或建议，请通过以下方式联系：

- 提交 Issue 到项目仓库
- 查看系统日志进行调试
- 参考本文档的测试用例

---

**文档版本**: v2.0  
**最后更新**: 2025 年 12 月 6 日  
**更新内容**: 优化排序算法，改为优先级排序（完全匹配 > 部分匹配 > 无匹配）  
**作者**: 智能问卷分析系统开发团队
