...
- 通过关联 Survey 表获取问卷标题，不冗余存储
- `timestamps: false`：没有 `createdAt` 和 `updatedAt` 字段
...
| userId    | VARCHAR(50)  | NOT NULL, FK | -                 | 用户 ID (关联 users.id)   |
| surveyId  | VARCHAR(50)  | NOT NULL, FK | -                 | 问卷 ID (关联 surveys.id) |
| rating    | DECIMAL(2,1) | -            | NULL              | 评分 (1-5)                |
...
- 通过关联 User 表获取用户信息（username, avatar），不冗余存储
- 删除操作为真删除，不使用软删除
...
- `updatedAt: false`: 收藏记录不需要更新时间，Model 中已禁用
...
- 2025-12-07: 删除了 `description` 字段（前端未使用）
- 2025-12-07: 删除了 `displayOrder` 字段（所有值相同，无实际意义）
- 2025-12-07: 删除了 `createdAt` 和 `updatedAt` 字段（前端无创建分类功能，无意义）
- 2025-12-07: 设置 `timestamps: false`（不再自动管理时间戳字段）
...
| description | VARCHAR(500) | -        | NULL   | 徽章描述                                                                                                                                                                                   |
| icon        | VARCHAR(50)  | -        | NULL   | 图标(emoji)                                                                                                                                                                                |
| requirement | INT          | NOT NULL | 1      | 解锁条件(数值)                                                                                                                                                                             |
| type        | VARCHAR(50)  | NOT NULL | -      | 徽章类型: login_count, continuous_login, survey_count, create_count, publish_count, approved_count, favorite_count, comment_count, share_count, profile_complete, total_points, user_level |
| points      | INT          | NOT NULL | 0      | 获得该徽章奖励的积分                                                                                                                                                                       |
...
- `timestamps: false`: 徽章数据为系统预设，不需要时间戳字段
...
- 2025-12-07: 模型设置为 `timestamps: false`（徽章为系统预设数据）
- 2025-12-07: 字段类型调整以匹配实际 Model 定义
...
| reason    | VARCHAR(200) | NOT NULL     | -                 | 积分变动原因                      |
| type      | ENUM         | -            | 'earn'            | 类型: earn/spend                  |
| createdAt | DATETIME     | -            | CURRENT_TIMESTAMP | 创建时间                          |
...
- `type`: earn(获得积分) / spend(消费积分)
- `points`: 正数表示增加，负数表示减少
- 积分历史记录创建后不会修改，因此不需要 `updatedAt` 字段

**历史变更**

- 2025-12-07: 删除了 `updatedAt` 字段（积分历史不会更新）
- 2025-12-07: 字段名调整：`action`→`reason`, `description` 合并到 `reason`
- 2025-12-07: 设置 `timestamps: false`（只保留 createdAt）
...
| surveyTitle | VARCHAR(200) | -            | NULL              | 问卷标题                          |
...
- ✅ 删除 `surveys.rating` 字段（与 averageRating 重复，前端仅使用 averageRating）
- ✅ 删除 `surveys.thumbnail` 字段（后端定义但前后端从未使用）
- ✅ 删除 `categories.description` 字段（前端未使用）
- ✅ 删除 `categories.displayOrder` 字段（所有值相同为 0，无实际意义）
- ✅ 删除 `categories.createdAt` 和 `updatedAt` 字段（前端无创建分类功能）
- ✅ 删除 `point_histories.updatedAt` 字段（积分历史记录不会更新）
- ✅ 更新相关代码：
  - 模型文件：`server/models/Survey.js`
  - 模型文件：`server/models/Category.js`（设置 timestamps: false）
  - 模型文件：`server/models/PointHistory.js`（设置 timestamps: false）
  - 控制器：`server/controllers/categoryController.js`（改用 name 排序）
- ✅ 迁移脚本：`server/migrations/remove-unused-fields.sql`

**字段保留说明**

- ✅ 保留 `surveys.settings` 字段（后端创建问卷时有使用）
- ✅ 保留 `surveys.category` 和 `categoryId` 字段（前端同时使用两者）
- ✅ 保留 `surveys.duration` 字段（前端多处使用）
- ✅ 保留 `surveys.averageRating` 字段（前端大量使用）
- ✅ 保留 `answers.surveyTitle` 字段（前端答题记录页面使用）
- ✅ 保留 `reports.surveyTitle` 字段（前端报告页面使用）
- ✅ 保留 `reports.answerId` 字段（用于关联具体答案记录）
