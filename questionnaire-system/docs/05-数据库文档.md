...
| tags | JSON | - | NULL | 用户标签 |
| completedSurveys | JSON | - | NULL | 已完成的问卷ID列表 |
| continuousLoginDays | INT | - | 0 | 连续登录天数 |
| loginCount | INT | - | 0 | 登录次数 |
| unlockedBadges | JSON | - | NULL | 已解锁的徽章ID列表 |
...
| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | VARCHAR(50) | PK | - | 问卷ID |
| userId | VARCHAR(50) | NOT NULL, FK | - | 创建者ID (关联 users.id) |
| title | VARCHAR(200) | NOT NULL | - | 问卷标题 |
| description | TEXT | - | NULL | 问卷描述 |
| category | VARCHAR(100) | - | NULL | 分类名称 |
| categoryId | VARCHAR(50) | FK | NULL | 分类ID (关联 categories.id) |
| tags | JSON | - | NULL | 标签 |
...
| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | VARCHAR(50) | PK | - | 答案ID |
| userId | VARCHAR(50) | NOT NULL, FK | - | 用户ID (关联 users.id) |
| surveyId | VARCHAR(50) | NOT NULL, FK | - | 问卷ID (关联 surveys.id) |
...
| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | VARCHAR(50) | PK | - | 关联ID |
| userId | VARCHAR(50) | NOT NULL, FK | - | 用户ID (关联 users.id) |
| badgeId | VARCHAR(50) | NOT NULL, FK | - | 徽章ID (关联 badges.id) |
| unlockedAt | DATETIME | - | CURRENT_TIMESTAMP | 解锁时间 |
| createdAt | DATETIME | - | CURRENT_TIMESTAMP | 创建时间 |
| updatedAt | DATETIME | - | CURRENT_TIMESTAMP | 更新时间 |
...
| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | VARCHAR(50) | PK | - | 日志ID |
| adminId | VARCHAR(50) | NOT NULL, FK | - | 管理员ID (关联 users.id) |
| action | VARCHAR(200) | NOT NULL | - | 操作描述 |
| targetType | VARCHAR(50) | - | NULL | 目标类型 (survey/user/comment等) |
| targetId | VARCHAR(50) | - | NULL | 目标ID |
...
| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | VARCHAR(50) | PK | - | 数据ID |
| userId | VARCHAR(50) | NOT NULL, FK | - | 用户ID (关联 users.id) |
| activityType | VARCHAR(50) | NOT NULL | - | 活动类型 |
| data | JSON | - | NULL | 活动数据 |
...
| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | VARCHAR(50) | PK | - | 公告ID |
| title | VARCHAR(200) | NOT NULL | - | 公告标题 |
| content | TEXT | NOT NULL | - | 公告内容 |
| type | VARCHAR(50) | - | 'info' | 类型: info/warning/success |
| priority | INT | - | 0 | 优先级 |
| createdAt | DATETIME | - | CURRENT_TIMESTAMP | 创建时间 |
| updatedAt | DATETIME | - | CURRENT_TIMESTAMP | 更新时间 |
...
| 字段名 | 类型 | 约束 | 默认值 | 说明 |
|--------|------|------|--------|------|
| id | VARCHAR(50) | PK | - | 回收站记录ID |
| userId | VARCHAR(50) | NOT NULL, FK | - | 用户ID (关联 users.id) |
| itemType | VARCHAR(50) | NOT NULL | - | 项目类型 (survey/answer等) |
| itemId | VARCHAR(50) | NOT NULL | - | 项目ID |
| itemData | JSON | - | NULL | 项目数据 (备份) |
| deletedAt | DATETIME | - | CURRENT_TIMESTAMP | 删除时间 |
| createdAt | DATETIME | - | CURRENT_TIMESTAMP | 创建时间 |
| updatedAt | DATETIME | - | CURRENT_TIMESTAMP | 更新时间 |
...
9. **UserBadges.userId → users.id**
   - 多对多关系 (用户 ↔ 徽章)
   - 级联操作: ON DELETE CASCADE

10. **UserBadges.badgeId → badges.id**
    - 多对多关系
    - 级联操作: ON DELETE CASCADE

11. **PointHistories.userId → users.id**
    - 一个用户有多条积分记录
    - 级联操作: ON DELETE CASCADE

12. **Reports.userId → users.id**
    - 一个用户可以有多个报告
    - 级联操作: ON DELETE CASCADE

13. **Reports.surveyId → surveys.id**
    - 一个问卷可以生成多个报告
    - 级联操作: ON DELETE CASCADE

14. **AdminActivities.adminId → users.id**
    - 一个管理员有多条操作记录
    - 级联操作: ON DELETE CASCADE
...
- JSON: 存储复杂结构 (questionList, answers, tags等)
- ENUM: 限定值范围 (status, role等)
...
1. **ID 生成**: 使用 `generateId()` 函数生成唯一ID，格式如 `user_1701234567890`
2. **时区**: 数据库时区设置为 UTC+8
