...
***时序图最新描述***
    用户->>前端已发布问卷页面: ① 点击"查看数据"按钮
    前端已发布问卷页面->>前端List.vue: ② 触发viewResults函数
    前端List.vue->>API层(survey.js): ③ 调用获取答案接口
    ...
    控制器(answerController.js)-->>前端List.vue: ⑧ 返回答案数据
    前端List.vue->>前端StatsDialog.vue: ⑨ 传递数据并打开统计对话框
