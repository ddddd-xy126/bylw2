...
***时序图最新描述***
    用户->>前端页面: ① 进入问卷详情页面
    前端页面->>API层(questionnaire.js): ② 触发加载评论列表
    API层(questionnaire.js)->>后端路由(comments.js): ③ GET /comments/survey/:surveyId
    后端路由(comments.js)->>控制器(commentController.js): ④ 查询评论数据
    控制器(commentController.js)->>数据库: ⑤ 查询Comment表并关联User表
    数据库-->>控制器(commentController.js): ⑥ 返回评论列表
    控制器(commentController.js)-->>前端页面: ⑦ 返回评论数据
    前端页面->>UI: ⑧ 渲染评论列表
    用户->>前端页面: ⑨ 点击"发表评论"
    前端页面->>API层(questionnaire.js): ⑩ 提交评论与评分
    ...
    控制器(commentController.js)-->>前端页面: ⑮ 返回新评论数据
    前端页面->>UI: ⑯ 刷新评论列表并提示成功
    用户->>前端页面: ⑰ 点击"删除评论"
    前端页面->>API层(questionnaire.js): ⑱ 发送删除请求
    ...
    控制器(commentController.js)-->>前端页面: ㉓ 返回成功标识
    前端页面->>UI: ㉔ 刷新评论列表并提示成功
***end***
