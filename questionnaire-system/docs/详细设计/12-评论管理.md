...
**_时序图最新描述_**
用户->>DetailPage.vue: ① 进入问卷详情页面
前端页面 DetailPage.vue->>API 层(questionnaire.js): ② 触发加载评论列表
API 层(questionnaire.js)->>后端路由(comments.js): ③ GET /comments/survey/:surveyId
后端路由(comments.js)->>控制器(commentController.js): ④ 查询评论数据
控制器(commentController.js)->>数据库: ⑤ 查询 Comment 表并关联 User 表
数据库-->>控制器(commentController.js): ⑥ 返回评论列表
控制器(commentController.js)-->>DetailPage.vue: ⑦ 返回评论数据

    用户->>ResultPage.vue: ⑨ 点击"发表评论"
    ResultPage.vue->>API层(questionnaire.js): ⑩ 提交评论与评分
    API层(questionnaire.js)->>后端路由(comments.js): ⑪ POST /comments/survey/:surveyId
    后端路由(comments.js)->>控制器(commentController.js): ⑫ 创建评论记录(事务)
    控制器(commentController.js)->>数据库: ⑬ 创建Comment记录并更新问卷评分
    数据库-->>控制器(commentController.js): ⑭ 返回创建结果
    控制器(commentController.js)-->>ResultPage.vue: ⑮ 返回新评论数据

    用户->>CommentsDialog.vue: ⑰ 点击"删除评论"
    CommentsDialog.vue->>API层(questionnaire.js): ⑱ 发送删除请求
    API层(questionnaire.js)->>后端路由(comments.js): ⑲ DELETE /comments/:id
    后端路由(comments.js)->>控制器(commentController.js): ⑳ 验证权限并删除评论
    控制器(commentController.js)->>数据库: ㉑ 删除Comment记录
    数据库-->>控制器(commentController.js): ㉒ 返回删除结果
    控制器(commentController.js)-->>CommentsDialog.vue: ㉓ 返回成功标识
