...
用户 → 前端页面: 点击收藏按钮
前端页面 → useHomeLogic.js: 调用 toggleFavorite
useHomeLogic.js → userStore: 检查收藏状态
userStore → useHomeLogic.js: 返回收藏状态
useHomeLogic.js → 后端 API: 调用对应 API（已收藏 → 删除，未收藏 → 添加）
后端 API → MySQL: 更新收藏记录和计数
MySQL → 后端 API: 返回操作结果
后端 API → useHomeLogic.js: 返回操作结果
useHomeLogic.js → userStore: 更新本地收藏状态
useHomeLogic.js → 用户界面: 显示操作成功提示
**_end_**
