...
useQuestionnaireEditor.js → localStorage: 检查本地草稿
 localStorage → useQuestionnaireEditor.js: 返回本地草稿数据结果
useQuestionnaireEditor.js → QuestionnaireEditor.vue: 初始化表单数据，启动 30 秒自动保存

用户 → QuestionnaireEditor.vue: 编辑题目和选项
QuestionnaireEditor.vue → useQuestionnaireEditor.js: 更新题目数组

用户 → QuestionnaireEditor.vue: 点击"保存草稿"/"发布问卷"
QuestionnaireEditor.vue → useQuestionnaireEditor.js: 验证并构建草稿数据
useQuestionnaireEditor.js → 后端 API: 保存草稿（status="draft"）/发布问卷（status="published"/"pending"）
后端 API->>数据库: ⑩ 保存问卷到 surveys 表
数据库-->>后端 API: ⑪ 返回保存结果
后端 API-->>前端 useQuestionnaireEditor.js: ⑫ 返回成功响应
useQuestionnaireEditor.js → localStorage: 清除本地缓存
