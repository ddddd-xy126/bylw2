...
***时序图最新描述***
    用户->>前端QuestionnaireEditor.vue: ① 进入创建/编辑页面
    前端QuestionnaireEditor.vue->>前端useQuestionnaireEditor.js: ② 调用initialize初始化
    前端useQuestionnaireEditor.js->>API层(category.js): ③ 加载分类数据
    前端useQuestionnaireEditor.js->>localStorage: ④ 检查本地草稿
    前端useQuestionnaireEditor.js->>前端QuestionnaireEditor.vue: ⑤ 初始化表单并启动30秒自动保存
    用户->>前端QuestionnaireEditor.vue: ⑥ 编辑题目和选项
    前端QuestionnaireEditor.vue->>前端useQuestionnaireEditor.js: ⑦ 更新题目数组
    用户->>前端QuestionnaireEditor.vue: ⑧ 点击"保存草稿"
    前端useQuestionnaireEditor.js->>后端API: ⑨ POST/PUT /surveys (status="draft")
    后端API->>数据库: ⑩ 保存问卷到surveys表
    数据库-->>后端API: ⑪ 返回保存结果
    后端API-->>前端useQuestionnaireEditor.js: ⑫ 返回成功响应
    前端useQuestionnaireEditor.js->>localStorage: ⑬ 清除本地缓存
    前端useQuestionnaireEditor.js->>路由器: ⑭ 跳转到问卷管理页面
***end***
