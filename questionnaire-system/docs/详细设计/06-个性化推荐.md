个性化推荐
1、简要描述
（1）功能描述：根据用户的兴趣标签智能推荐相关问卷，帮助用户快速找到感兴趣的内容。系统通过分析用户标签与问卷的标签进行匹配，计算相关性得分并结合问卷质量和热度进行综合排序。

（2）代码逻辑：
用户在首页选择"推荐"排序时，系统从Pinia store获取当前登录用户的标签信息。系统遍历所有问卷，计算每个问卷与用户标签的匹配度：统计问卷标签中有多少个在用户标签列表中，得到匹配数量matchCount，并判断是否完全匹配。系统根据标签匹配度、问卷评分和参与人数计算综合推荐得分，公式为：推荐得分=标签匹配度×0.5 +评分得分×0.3 +热度得分×0.2。其中，标签匹配度=匹配标签数/用户标签总数，评分得分=问卷评分/ 5，热度得分= min(参与人数/ 1000, 1)。系统按照优先级排序：完全匹配的问卷排最前，部分匹配的问卷按匹配数量降序排列，匹配数量相同时按综合得分降序排列。若用户未设置标签，系统随机打乱问卷顺序展示。排序后的问卷列表渲染到页面，用户看到的是与自己兴趣最相关的推荐问卷。
...
***时序图最新描述***
    用户->>前端List.vue: ① 选择"推荐"排序方式
    前端List.vue->>前端useListFilter.js: ② 触发推荐排序逻辑
    前端useListFilter.js->>Pinia(userStore): ③ 获取用户标签
    Pinia(userStore)-->>前端useListFilter.js: ④ 返回用户标签数组
    前端useListFilter.js->>前端useListFilter.js: ⑤ 遍历问卷计算标签匹配度
    前端useListFilter.js->>前端useListFilter.js: ⑥ 计算综合推荐得分(标签50%+评分30%+热度20%)
    前端useListFilter.js->>前端useListFilter.js: ⑦ 按优先级排序(完全匹配>匹配数量>综合得分)
    前端useListFilter.js-->>前端List.vue: ⑧ 返回排序后的问卷列表
    前端List.vue-->>用户: ⑨ 渲染推荐问卷列表
***end***
