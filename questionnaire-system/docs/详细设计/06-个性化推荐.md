...
（2）代码逻辑：
用户在首页或问卷列表页面选择"推荐"排序方式时，系统从 Pinia store 获取当前登录用户的标签信息（userStore.profile.tags）。系统遍历所有问卷，计算每个问卷与用户标签的匹配度：统计问卷标签中有多少个在用户标签列表中，得到匹配数量 matchCount，并判断是否完全匹配（用户所有标签都在问卷标签中）。系统根据标签匹配度、问卷评分和参与人数计算综合推荐得分，公式为：推荐得分 = 标签匹配度 × 0.5 + 评分得分 × 0.3 + 热度得分 × 0.2。其中，标签匹配度 = 匹配标签数 / 用户标签总数，评分得分 = 问卷评分 / 5，热度得分 = min(参与人数 / 1000, 1)。系统按照优先级排序：完全匹配的问卷排最前，部分匹配的问卷按匹配数量降序排列，匹配数量相同时按综合得分降序排列。若用户未设置标签，系统随机打乱问卷顺序展示。排序后的问卷列表渲染到页面，用户看到的是与自己兴趣最相关的推荐问卷。

###个性化推荐时序图描述
用户 → HomePage.vue / List.vue: 选择"推荐"排序方式
组件 → useListFilter.js: 触发排序逻辑（sortBy = "recommended"）
useListFilter.js → Pinia store: 获取用户标签（userStore.profile.tags）
Pinia store → useListFilter.js: 返回用户标签数组
useListFilter.js → useListFilter.js: 遍历问卷列表计算匹配度
useListFilter.js → useListFilter.js: 计算每个问卷的标签匹配数（matchCount）
useListFilter.js → useListFilter.js: 判断是否完全匹配（isFullMatch）
useListFilter.js → useListFilter.js: 计算综合推荐得分（recommendScore）
