...
***时序图最新描述***
    用户->>前端List.vue: ① 访问问卷列表页
    前端List.vue->>前端List.vue: ② onMounted调用loadData
    前端List.vue->>API层(survey.js + category.js): ③ 并发请求问卷列表和分类列表
    API层(survey.js + category.js)->>控制器(surveyController + categoryController): ④ 查询问卷和分类
    控制器(surveyController + categoryController)->>数据库: ⑤ 查询surveys表(published状态)
    数据库-->>控制器(surveyController + categoryController): ⑥ 返回问卷数据
    控制器(surveyController + categoryController)->>数据库: ⑦ 查询categories表
    数据库-->>控制器(surveyController + categoryController): ⑧ 返回分类数据
    控制器(surveyController + categoryController)-->>前端: ⑨ 返回问卷+分类
    前端List.vue->>前端List.vue: ⑩ 存储至Pinia状态
    前端List.vue-->>用户: ⑪ 渲染问卷列表
    
    用户->>前端List.vue: ⑫ 搜索或筛选
    前端List.vue->>前端List.vue: ⑬ 实时过滤数据
    前端List.vue-->>用户: ⑭ 更新显示结果