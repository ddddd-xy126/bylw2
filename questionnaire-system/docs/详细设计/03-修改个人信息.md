...
###修改个人信息时序图描述
用户 → InfoPage.vue: 执行修改操作（基本信息/头像/密码） 
InfoPage.vue → user.js: 调用 updateProfileApi
user.js → 后端路由 (PUT /users/:id): 发送 HTTP PUT 请求
后端路由 (routes/user.js) → userController.js: 转发到 userController.updateProfile
userController.js → userController.js: 校验用户登录状态
userController.js → MySQL: 更新用户信息（基本资料/头像/密码）
MySQL → userController.js: 返回更新结果
userController.js → userController.js: 检查资料完整度并发放积分（如满足条件）
userController.js → user.js: 返回更新成功响应
user.js → InfoPage.vue: 返回更新结果
InfoPage.vue → Pinia: 更新本地用户数据
InfoPage.vue → 用户界面: 显示成功提示并结束编辑流程
