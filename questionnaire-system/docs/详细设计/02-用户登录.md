...
LoginPage.vue → auth.js: 调用 login API
auth.js → 后端路由 (POST /auth/login): 发送 HTTP POST 请求
后端路由 (routes/auth.js) → validateLogin 中间件: 验证数据格式（用户名、密码）
validateLogin 中间件 → authController.js: 验证通过，转发到 authController.login
authController.js → MySQL: 查询用户（通过用户名或邮箱）
MySQL → authController.js: 返回用户记录
authController.js → authController.js: 使用 bcrypt 验证密码（调用 user.validatePassword）
authController.js → authController.js: 检查账号状态（是否被封禁）
authController.js → authController.js: 生成 JWT Token（包含用户 ID 和角色，有效期 7 天）
authController.js → auth.js: 返回登录成功响应（Token 和用户信息）
auth.js → LoginPage.vue: 返回登录结果
LoginPage.vue → localStorage: 存储 JWT Token
LoginPage.vue → Pinia store: 存储用户信息
LoginPage.vue → LoginPage.vue: 触发登录后业务逻辑（积分奖励、退回问卷提醒）
LoginPage.vue → 用户界面: 根据角色跳转页面（管理员 →/admin/dashboard，普通用户 →/home）
