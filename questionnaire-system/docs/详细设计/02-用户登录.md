...
###用户登录时序图描述
1.用户在 LoginPage.vue 填写用户名/邮箱和密码，点击登录
2.LoginPage.vue 向 auth.js 调用 login API
3.auth.js 发送 HTTP 请求给 validateLogin.js 中间件进行数据验证（用户名、密码）
4.validateLogin.js 验证通过后返回结果给 auth.js
5.auth.js 调用登录控制器 authController.js
6.authController.js 查询数据库（通过用户名或邮箱查找用户）
7.MySQL 数据库返回用户记录给 authController.js
8.authController.js 使用 bcrypt 验证密码
9.authController.js 检查账号状态（是否被封禁）
10.authController.js 生成 JWT Token（包含用户 ID 和角色）
11.authController.js 返回登录结果（Token 和用户信息）给 auth.js
12.auth.js 返回结果给 LoginPage.vue
13.LoginPage.vue 存储 Token 到 localStorage
14.LoginPage.vue 存储用户信息到 Pinia store
15.LoginPage.vue 触发登录后业务逻辑（积分奖励、退回问卷提醒）
16.LoginPage.vue 根据用户角色跳转到对应页面
