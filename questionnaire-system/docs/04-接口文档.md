# 问卷系统 - API 接口文档

## 📋 概述

本文档描述问卷系统后端 API 的所有接口，包括请求方式、参数、响应格式等。

### 基础信息
- **Base URL**: `http://localhost:3000/api`
- **数据格式**: JSON
- **认证方式**: JWT Bearer Token
- **字符编码**: UTF-8

### 通用响应格式

#### 成功响应
```json
{
  "success": true,
  "data": { ... },
  "message": "操作成功"
}
```

#### 错误响应
```json
{
  "success": false,
  "message": "错误信息",
  "errors": [ ... ]
}
```

### HTTP 状态码
- `200` - 成功
- `201` - 创建成功
- `400` - 请求参数错误
- `401` - 未认证
- `403` - 无权限
- `404` - 资源不存在
- `500` - 服务器错误

---

## 🔐 认证接口 (`/api/auth`)

### 1. 用户注册

**POST** `/auth/register`

**请求头**
```
Content-Type: application/json
```

**请求体**
```json
{
  "username": "testuser",
  "password": "password123",
  "email": "test@example.com",
  "nickname": "测试用户"
}
```

**参数说明**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| username | string | ✅ | 用户名，3-20字符，字母数字下划线 |
| password | string | ✅ | 密码，至少6字符 |
| email | string | ❌ | 邮箱 |
| nickname | string | ❌ | 昵称，默认为用户名 |

**响应**
```json
{
  "success": true,
  "data": {
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
    "user": {
      "id": "user_123456",
      "username": "testuser",
      "nickname": "测试用户",
      "email": "test@example.com",
      "role": "user",
      "points": 100,
      "avatar": null
    }
  }
}
```

---

### 2. 用户登录

**POST** `/auth/login`

**请求体**
```json
{
  "username": "testuser",
  "password": "password123"
}
```

**参数说明**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| username | string | ✅ | 用户名 |
| password | string | ✅ | 密码 |

**响应**
```json
{
  "success": true,
  "data": {
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
    "user": {
      "id": "user_123456",
      "username": "testuser",
      "nickname": "测试用户",
      "role": "user",
      "points": 150,
      "level": 1
    }
  }
}
```

---

### 3. 获取当前用户信息

**GET** `/auth/me`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "user_123456",
    "username": "testuser",
    "nickname": "测试用户",
    "email": "test@example.com",
    "role": "user",
    "points": 150,
    "level": 1,
    "avatar": "https://...",
    "createdAt": "2024-01-01T00:00:00.000Z"
  }
}
```

---

### 4. 登出

**POST** `/auth/logout`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "message": "登出成功"
}
```

---

### 5. 刷新 Token

**POST** `/auth/refresh`

**请求头**
```
Authorization: Bearer {old_token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "token": "new_token_here"
  }
}
```

---

## 👤 用户接口 (`/api/users`)

### 1. 获取用户列表

**GET** `/users`

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| page | number | ❌ | 页码，默认1 |
| limit | number | ❌ | 每页数量，默认10 |
| sort | string | ❌ | 排序字段，默认 createdAt |
| order | string | ❌ | 排序方式，asc/desc，默认 desc |

**响应**
```json
{
  "success": true,
  "data": {
    "users": [
      {
        "id": "user_123",
        "username": "user1",
        "nickname": "用户1",
        "avatar": "https://...",
        "points": 200,
        "level": 2,
        "role": "user"
      }
    ],
    "total": 100,
    "page": 1,
    "totalPages": 10
  }
}
```

---

### 2. 获取单个用户信息

**GET** `/users/:id`

**响应**
```json
{
  "success": true,
  "data": {
    "id": "user_123",
    "username": "testuser",
    "nickname": "测试用户",
    "avatar": "https://...",
    "bio": "个人简介",
    "points": 200,
    "level": 2,
    "gender": "male",
    "age": 25,
    "city": "北京",
    "profession": "工程师",
    "tags": ["技术", "阅读"],
    "createdAt": "2024-01-01T00:00:00.000Z"
  }
}
```

---

### 3. 更新用户信息

**PATCH** `/users/:id`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**
```json
{
  "nickname": "新昵称",
  "bio": "新的个人简介",
  "avatar": "https://new-avatar.jpg",
  "gender": "female",
  "age": 26,
  "city": "上海",
  "profession": "设计师",
  "tags": ["设计", "艺术"]
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "user_123",
    "nickname": "新昵称",
    "bio": "新的个人简介",
    // ...其他字段
  }
}
```

---

### 4. 修改密码

**PUT** `/users/password`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**
```json
{
  "oldPassword": "old_password",
  "newPassword": "new_password"
}
```

**响应**
```json
{
  "success": true,
  "message": "密码修改成功"
}
```

---

### 5. 获取用户统计

**GET** `/users/stats/:id?`

**请求头**
```
Authorization: Bearer {token}
```

**说明**: 不传 id 则获取当前用户的统计

**响应**
```json
{
  "success": true,
  "data": {
    "createdSurveys": 10,
    "completedSurveys": 50,
    "points": 500,
    "level": 5,
    "badges": 8,
    "favorites": 15
  }
}
```

---

### 6. 获取用户的问卷列表

**GET** `/users/surveys/:id?`

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| page | number | ❌ | 页码 |
| limit | number | ❌ | 每页数量 |
| status | string | ❌ | 状态筛选: draft/pending/published |

**响应**
```json
{
  "success": true,
  "data": {
    "surveys": [...],
    "total": 10,
    "page": 1,
    "totalPages": 1
  }
}
```

---

### 7. 获取用户的答题记录

**GET** `/users/answers/:id?`

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| page | number | ❌ | 页码 |
| limit | number | ❌ | 每页数量 |

**响应**
```json
{
  "success": true,
  "data": {
    "answers": [
      {
        "id": "answer_123",
        "surveyId": "survey_456",
        "surveyTitle": "问卷标题",
        "score": 85,
        "submittedAt": "2024-01-01T10:00:00.000Z"
      }
    ],
    "total": 20,
    "page": 1,
    "totalPages": 2
  }
}
```

---

### 8. 获取用户的收藏列表

**GET** `/users/favorites/:id?`

**响应**
```json
{
  "success": true,
  "data": {
    "favorites": [
      {
        "id": "fav_123",
        "surveyId": "survey_456",
        "survey": {
          "id": "survey_456",
          "title": "问卷标题",
          "category": "学术调研",
          "rating": 4.5
        },
        "createdAt": "2024-01-01T10:00:00.000Z"
      }
    ],
    "total": 15
  }
}
```

---

### 9. 获取积分历史

**GET** `/users/points/:id?`

**响应**
```json
{
  "success": true,
  "data": {
    "history": [
      {
        "id": "point_123",
        "points": 10,
        "action": "完成问卷",
        "description": "完成问卷《xxx》",
        "createdAt": "2024-01-01T10:00:00.000Z"
      }
    ],
    "total": 50
  }
}
```

---

## 📝 问卷接口 (`/api/surveys`)

### 1. 获取问卷列表

**GET** `/surveys`

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| page | number | ❌ | 页码，默认1 |
| limit | number | ❌ | 每页数量，默认10 |
| category | string | ❌ | 分类筛选 |
| keyword | string | ❌ | 搜索关键词 |
| status | string | ❌ | 状态: published/draft/pending |
| sort | string | ❌ | 排序: createdAt/rating/participants |

**响应**
```json
{
  "success": true,
  "data": {
    "surveys": [
      {
        "id": "survey_123",
        "title": "问卷标题",
        "description": "问卷描述",
        "category": "学术调研",
        "questions": 10,
        "participants": 100,
        "rating": 4.5,
        "status": "published",
        "creator": {
          "id": "user_456",
          "username": "author",
          "nickname": "作者"
        },
        "createdAt": "2024-01-01T00:00:00.000Z",
        "publishedAt": "2024-01-02T00:00:00.000Z"
      }
    ],
    "total": 50,
    "page": 1,
    "totalPages": 5
  }
}
```

---

### 2. 获取问卷详情

**GET** `/surveys/:id`

**响应**
```json
{
  "success": true,
  "data": {
    "id": "survey_123",
    "title": "问卷标题",
    "description": "问卷描述",
    "category": "学术调研",
    "tags": ["标签1", "标签2"],
    "questionList": [
      {
        "id": "q1",
        "type": "choice",
        "title": "题目1",
        "options": ["选项A", "选项B", "选项C"],
        "required": true
      },
      {
        "id": "q2",
        "type": "text",
        "title": "题目2",
        "required": false
      }
    ],
    "settings": {
      "allowAnonymous": true,
      "showProgress": true,
      "randomOrder": false
    },
    "participants": 100,
    "responses": 95,
    "rating": 4.5,
    "ratingCount": 20,
    "favoriteCount": 15,
    "creator": {
      "id": "user_456",
      "username": "author",
      "nickname": "作者",
      "avatar": "https://..."
    },
    "createdAt": "2024-01-01T00:00:00.000Z",
    "publishedAt": "2024-01-02T00:00:00.000Z"
  }
}
```

---

### 3. 创建问卷

**POST** `/surveys`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**
```json
{
  "title": "新问卷标题",
  "description": "问卷描述",
  "category": "学术调研",
  "categoryId": "cat_123",
  "tags": ["标签1", "标签2"],
  "questionList": [
    {
      "id": "q1",
      "type": "choice",
      "title": "您的性别是？",
      "options": ["男", "女", "其他"],
      "required": true
    },
    {
      "id": "q2",
      "type": "text",
      "title": "您的建议",
      "required": false
    }
  ],
  "settings": {
    "allowAnonymous": true,
    "showProgress": true,
    "randomOrder": false
  },
  "status": "draft"
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "survey_new_123",
    "title": "新问卷标题",
    "status": "draft",
    // ...其他字段
  }
}
```

---

### 4. 更新问卷

**PUT** `/surveys/:id`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**: 同创建问卷

**响应**
```json
{
  "success": true,
  "data": {
    "id": "survey_123",
    // ...更新后的字段
  }
}
```

---

### 5. 删除问卷

**DELETE** `/surveys/:id`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "message": "问卷已删除"
}
```

---

### 6. 获取问卷统计

**GET** `/surveys/:id/stats`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "surveyId": "survey_123",
    "title": "问卷标题",
    "totalResponses": 100,
    "completionRate": 0.95,
    "averageScore": 85.5,
    "averageDuration": 300,
    "questionStats": [
      {
        "questionId": "q1",
        "questionTitle": "您的性别是？",
        "type": "choice",
        "totalAnswers": 100,
        "distribution": {
          "男": 60,
          "女": 35,
          "其他": 5
        }
      }
    ]
  }
}
```

---

### 7. 收藏/取消收藏问卷

**POST** `/surveys/:id/favorite`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "favorited": true
  }
}
```

---

### 8. 检查是否收藏

**GET** `/surveys/:id/favorite/check`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "favorited": true
  }
}
```

---

### 9. 获取用户创建的问卷

**GET** `/surveys/user/:userId`

**请求头**
```
Authorization: Bearer {token}
```

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| status | string | ❌ | 状态筛选 |
| page | number | ❌ | 页码 |
| limit | number | ❌ | 每页数量 |

**响应**: 同获取问卷列表

---

## ✍️ 答案接口 (`/api/answers`)

### 1. 提交答案

**POST** `/answers`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**
```json
{
  "surveyId": "survey_123",
  "surveyTitle": "问卷标题",
  "answers": [
    {
      "questionId": "q1",
      "question": "您的性别是？",
      "answer": "男",
      "text": "男"
    },
    {
      "questionId": "q2",
      "question": "您的建议",
      "answer": "建议内容",
      "text": "建议内容"
    }
  ],
  "score": 85,
  "result": "良好",
  "duration": 300
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "answer_new_123",
    "surveyId": "survey_123",
    "userId": "user_456",
    "score": 85,
    "submittedAt": "2024-01-01T10:00:00.000Z"
  }
}
```

---

### 2. 获取答案列表

**GET** `/answers`

**请求头**
```
Authorization: Bearer {token}
```

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| userId | string | ❌ | 按用户筛选 |
| surveyId | string | ❌ | 按问卷筛选 |
| page | number | ❌ | 页码 |
| limit | number | ❌ | 每页数量 |

**响应**
```json
{
  "success": true,
  "data": {
    "answers": [...],
    "total": 20,
    "page": 1,
    "totalPages": 2
  }
}
```

---

### 3. 获取答案详情

**GET** `/answers/:id`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "answer_123",
    "surveyId": "survey_456",
    "surveyTitle": "问卷标题",
    "userId": "user_789",
    "answers": [...],
    "score": 85,
    "result": "良好",
    "duration": 300,
    "submittedAt": "2024-01-01T10:00:00.000Z"
  }
}
```

---

### 4. 获取用户在某问卷的答案

**GET** `/answers/survey/:surveyId`

**请求头**
```
Authorization: Bearer {token}
```

**响应**: 同获取答案详情

---

### 5. 检查用户是否已答过问卷

**GET** `/answers/survey/:surveyId/check`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "answered": true,
    "answerId": "answer_123"
  }
}
```

---

### 6. 删除答案

**DELETE** `/answers/:id`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "message": "答案已删除"
}
```

---

## 💬 评论接口 (`/api/comments`)

### 1. 获取问卷评论

**GET** `/comments/survey/:surveyId`

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| page | number | ❌ | 页码 |
| limit | number | ❌ | 每页数量 |

**响应**
```json
{
  "success": true,
  "data": {
    "comments": [
      {
        "id": "comment_123",
        "content": "评论内容",
        "rating": 5,
        "user": {
          "id": "user_456",
          "username": "user1",
          "nickname": "用户1",
          "avatar": "https://..."
        },
        "createdAt": "2024-01-01T10:00:00.000Z"
      }
    ],
    "total": 10,
    "page": 1,
    "totalPages": 1
  }
}
```

---

### 2. 创建评论

**POST** `/comments/survey/:surveyId`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**
```json
{
  "content": "这是一条评论",
  "rating": 5
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "comment_new_123",
    "surveyId": "survey_456",
    "userId": "user_789",
    "content": "这是一条评论",
    "rating": 5,
    "createdAt": "2024-01-01T10:00:00.000Z"
  }
}
```

---

### 3. 更新评论

**PUT** `/comments/:id`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**
```json
{
  "content": "修改后的评论",
  "rating": 4
}
```

**响应**: 同创建评论

---

### 4. 删除评论

**DELETE** `/comments/:id`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "message": "评论已删除"
}
```

---

## 🏷️ 分类接口 (`/api/categories`)

### 1. 获取所有分类

**GET** `/categories`

**响应**
```json
{
  "success": true,
  "data": [
    {
      "id": "cat_1",
      "name": "学术调研",
      "slug": "academic",
      "description": "学术研究相关问卷",
      "icon": "📚",
      "surveyCount": 50
    },
    {
      "id": "cat_2",
      "name": "市场调查",
      "slug": "market",
      "description": "市场调研问卷",
      "icon": "📊",
      "surveyCount": 30
    }
  ]
}
```

---

### 2. 获取单个分类

**GET** `/categories/:id`

**响应**
```json
{
  "success": true,
  "data": {
    "id": "cat_1",
    "name": "学术调研",
    "slug": "academic",
    "description": "学术研究相关问卷",
    "icon": "📚",
    "surveyCount": 50
  }
}
```

---

### 3. 通过 slug 获取分类

**GET** `/categories/slug/:slug`

**响应**: 同获取单个分类

---

### 4. 创建分类 (管理员)

**POST** `/categories`

**请求头**
```
Authorization: Bearer {admin_token}
```

**请求体**
```json
{
  "name": "新分类",
  "slug": "new-category",
  "description": "分类描述",
  "icon": "🎯"
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "cat_new",
    "name": "新分类",
    "slug": "new-category",
    // ...
  }
}
```

---

### 5. 更新分类 (管理员)

**PUT** `/categories/:id`

**请求头**
```
Authorization: Bearer {admin_token}
```

**请求体**: 同创建分类

**响应**: 同创建分类

---

### 6. 删除分类 (管理员)

**DELETE** `/categories/:id`

**请求头**
```
Authorization: Bearer {admin_token}
```

**响应**
```json
{
  "success": true,
  "message": "分类已删除"
}
```

---

## 🏅 徽章接口 (`/api/badges`)

### 1. 获取所有徽章

**GET** `/badges`

**响应**
```json
{
  "success": true,
  "data": [
    {
      "id": "badge_1",
      "name": "新手上路",
      "description": "完成第一个问卷",
      "icon": "🎖️",
      "requirement": 1,
      "type": "survey_completed"
    }
  ]
}
```

---

### 2. 获取单个徽章

**GET** `/badges/:id`

**响应**: 同获取所有徽章的单项

---

## 📊 报告接口 (`/api/reports`)

### 1. 生成报告

**POST** `/reports/generate`

**请求头**
```
Authorization: Bearer {token}
```

**请求体**
```json
{
  "surveyId": "survey_123",
  "surveyTitle": "问卷标题",
  "category": "心理测试",
  "answers": [
    {
      "question": "题目1",
      "text": "答案1"
    },
    {
      "question": "题目2",
      "text": "答案2"
    }
  ]
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "reportId": "report_123",
    "status": "completed",
    "content": "# 个人分析报告\n\n根据您的答案...",
    "generatedAt": "2024-01-01T10:05:00.000Z"
  }
}
```

---

### 2. 获取报告状态

**GET** `/reports/:id/status`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "report_123",
    "status": "completed",
    "generatedAt": "2024-01-01T10:05:00.000Z"
  }
}
```

**状态说明**
- `generating` - 生成中
- `completed` - 已完成
- `failed` - 生成失败

---

### 3. 获取报告详情

**GET** `/reports/:id`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "report_123",
    "userId": "user_456",
    "surveyId": "survey_789",
    "title": "心理测试 - 个人分析报告",
    "surveyTitle": "心理测试",
    "category": "心理测试",
    "content": "# 个人分析报告\n\n...",
    "status": "completed",
    "generatedAt": "2024-01-01T10:05:00.000Z",
    "createdAt": "2024-01-01T10:00:00.000Z"
  }
}
```

---

### 4. 获取用户所有报告

**GET** `/reports`

**请求头**
```
Authorization: Bearer {token}
```

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| page | number | ❌ | 页码 |
| limit | number | ❌ | 每页数量 |
| status | string | ❌ | 状态筛选 |

**响应**
```json
{
  "success": true,
  "data": {
    "reports": [...],
    "total": 5,
    "page": 1,
    "totalPages": 1
  }
}
```

---

### 5. 下载报告 (PDF)

**GET** `/reports/:id/download`

**请求头**
```
Authorization: Bearer {token}
```

**响应**: PDF 文件流

---

### 6. 删除报告

**DELETE** `/reports/:id`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "message": "报告已删除"
}
```

---

## 🔧 管理员接口 (`/api/admin`)

**所有管理员接口都需要管理员权限**

### 1. 仪表板统计

**GET** `/admin/dashboard/stats`

**请求头**
```
Authorization: Bearer {admin_token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "users": {
      "total": 1000,
      "today": 20
    },
    "surveys": {
      "total": 500,
      "pending": 10,
      "published": 450
    },
    "answers": {
      "total": 5000,
      "today": 100
    }
  }
}
```

---

### 2. 获取所有用户

**GET** `/admin/users`

**请求头**
```
Authorization: Bearer {admin_token}
```

**Query 参数**: page, limit

**响应**: 同 `/api/users`

---

### 3. 更新用户角色

**PUT** `/admin/users/:id/role`

**请求头**
```
Authorization: Bearer {admin_token}
```

**请求体**
```json
{
  "role": "admin"
}
```

**响应**
```json
{
  "success": true,
  "message": "用户角色已更新"
}
```

---

### 4. 删除用户

**DELETE** `/admin/users/:id`

**请求头**
```
Authorization: Bearer {admin_token}
```

**响应**
```json
{
  "success": true,
  "message": "用户已删除"
}
```

---

### 5. 审核问卷

**PUT** `/admin/surveys/:id/review`

**请求头**
```
Authorization: Bearer {admin_token}
```

**请求体**
```json
{
  "action": "approve",
  "reason": "审核通过"
}
```

**参数说明**
- `action`: "approve" (通过) 或 "reject" (拒绝)
- `reason`: 审核意见

**响应**
```json
{
  "success": true,
  "message": "问卷已通过审核"
}
```

---

### 6. 获取所有公告

**GET** `/admin/announcements`

**请求头**
```
Authorization: Bearer {admin_token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "announcements": [
      {
        "id": "ann_1",
        "title": "公告标题",
        "content": "公告内容",
        "type": "info",
        "createdAt": "2024-01-01T00:00:00.000Z"
      }
    ],
    "total": 5
  }
}
```

---

### 7. 创建公告

**POST** `/admin/announcements`

**请求头**
```
Authorization: Bearer {admin_token}
```

**请求体**
```json
{
  "title": "新公告",
  "content": "公告内容",
  "type": "info"
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "ann_new",
    "title": "新公告",
    // ...
  }
}
```

---

### 8. 更新公告

**PUT** `/admin/announcements/:id`

**请求头**
```
Authorization: Bearer {admin_token}
```

**请求体**: 同创建公告

**响应**: 同创建公告

---

### 9. 删除公告

**DELETE** `/admin/announcements/:id`

**请求头**
```
Authorization: Bearer {admin_token}
```

**响应**
```json
{
  "success": true,
  "message": "公告已删除"
}
```

---

### 10. 获取管理员活动日志

**GET** `/admin/activities`

**请求头**
```
Authorization: Bearer {admin_token}
```

**Query 参数**: page, limit

**响应**
```json
{
  "success": true,
  "data": {
    "activities": [
      {
        "id": "act_1",
        "adminId": "admin_123",
        "action": "审核问卷: 通过",
        "targetType": "survey",
        "targetId": "survey_456",
        "details": "审核通过",
        "createdAt": "2024-01-01T10:00:00.000Z"
      }
    ],
    "total": 100
  }
}
```

---

### 11. 创建活动日志

**POST** `/admin/activities`

**请求头**
```
Authorization: Bearer {admin_token}
```

**请求体**
```json
{
  "action": "操作描述",
  "targetType": "survey",
  "targetId": "survey_123",
  "details": "详细信息"
}
```

**响应**
```json
{
  "success": true,
  "data": {
    "id": "act_new",
    // ...
  }
}
```

---

## 📢 公告接口 (`/api/announcements`)

### 1. 获取公告列表 (公开)

**GET** `/announcements`

**Query 参数**
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| page | number | ❌ | 页码 |
| limit | number | ❌ | 每页数量 |
| type | string | ❌ | 类型筛选 |

**响应**: 同管理员获取所有公告

---

## 🗑️ 回收站接口 (`/api/recycleBin`)

### 1. 获取回收站列表

**GET** `/recycleBin`

**请求头**
```
Authorization: Bearer {token}
```

**响应**
```json
{
  "success": true,
  "data": {
    "items": [
      {
        "id": "recycle_1",
        "itemType": "survey",
        "itemId": "survey_123",
        "itemData": {...},
        "deletedAt": "2024-01-01T10:00:00.000Z"
      }
    ]
  }
}
```

---

## ⭐ 收藏接口 (`/api/favorites`)

### 1. 获取我的收藏

**GET** `/favorites`

**请求头**
```
Authorization: Bearer {token}
```

**响应**: 同 `/api/users/favorites`

---

## 📌 错误码说明

| 错误码 | 说明 | 示例 |
|--------|------|------|
| 400 | 请求参数错误 | 缺少必填字段 |
| 401 | 未认证 | Token 无效或过期 |
| 403 | 无权限 | 非管理员访问管理接口 |
| 404 | 资源不存在 | 问卷ID不存在 |
| 409 | 冲突 | 用户名已存在 |
| 500 | 服务器错误 | 数据库连接失败 |

---

## 🔒 认证说明

大部分接口需要在请求头中携带 JWT Token：

```
Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```

Token 有效期: 7天

---

## 📝 注意事项

1. 所有日期时间格式为 ISO 8601 (UTC)
2. 分页默认 page=1, limit=10
3. 文件上传大小限制: 10MB
4. API 调用频率限制: 100次/分钟 (可配置)
5. 所有接口支持 CORS

---

**文档版本**: v1.0  
**最后更新**: 2025年12月1日
