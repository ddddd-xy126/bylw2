          ...
          <template #default="{ row }">
            <div class="action-buttons">
              <el-button
                type="text"
                size="small"
                @click="previewQuestionnaire(row)"
              >
                <el-icon>
                  <View />
                </el-icon>
              ...
              <el-button
                type="text"
                size="small"
                @click="approveQuestionnaire(row)"
                class="success-btn"
              >
                <el-icon>
                  <Check />
...
const {
  ...
  searchFields: ["title", "submitterName"],
});
  ...
  try {
    ...
    if (reviewForm.result === "approve") {
      ...
      await recordAdminActivity({
        ...
        title: "审核通过问卷",
        description: `审核通过问卷"${currentQuestionnaire.value.title}"${
          reviewForm.comment ? "，意见：" + reviewForm.comment : ""
        }`,
      ...
      const survey = await apiClient.get(
        `/surveys/${currentQuestionnaire.value.id}`
      );
  ...
  try {
    ...
    pendingList.value = surveys.map((survey) => {
      const creatorName =
        ...
        survey.author ||
        "未知";
      const creatorEmail = survey.creator?.email || "";
      return {
        id: survey.id,
        title: survey.title,
        description: survey.description,
        submitterName: creatorName,
        submitterEmail: creatorEmail,
        submitterAvatar:
          survey.creator?.avatar ||
          `https://api.dicebear.com/7.x/initials/svg?seed=${creatorName}`,
