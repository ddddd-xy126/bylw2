  ...
  <div class="survey-section">
    ...
    <div v-loading="loading">
      ...
      <div class="featured-grid" v-if="surveys.length">
        <div v-for="survey in surveys" :key="survey.id" class="featured-card" @click="$emit('surveyClick', survey.id)">
          ...
          <div class="card-content">
            ...
            <div class="card-meta">
              ...
              <div class="meta-item rating">
                <el-rate :model-value="survey.rating || 0" disabled show-score text-color="#ff9900"
                  score-template="{value}" />
              </div>
            </div>

            <div class="card-tags">
              <el-tag v-for="tag in (survey.tags || []).slice(0, 3)" :key="tag" size="small" type="info" effect="plain">
                {{ tag }}
              </el-tag>
            </div>
          </div>
          ...
          <div class="card-footer">
            <el-button type="primary" size="small" @click.stop="$emit('surveyStart', survey.id)">
              开始测试
            </el-button>
            <el-button size="small" :type="isFavorite(survey.id) ? 'warning' : 'default'"
              @click.stop="$emit('toggleFavorite', survey.id)" v-if="showFavorite">
              <el-icon>
